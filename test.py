...
def get_values():
    ...
    global temp_dict
    dict_my = {}
    global dict_name_value
    global firebase_queue
    ...
    try:
        for i in range(105):
            ...
            if dict_my != dict_name_value:
                    ...
                    firebase_queue.put(dict_my)    
                    print("AAA : "+str(firebase_queue.qsize()))
...
def push_updates_to_firestore():
    ...
    while 1:
        try:
            # print("SSSSSS : "+str(firebase_queue.qsize()))
            if not firebase_queue.empty():
                dict_my = firebase_queue.get()

                if checkdict !=dict_my:
                    checkdict = dict_my
                    db_o.collection(u'CP Factory').document('Lathe Machine').set(
                    dict_my, merge=True)
        except Exception as e:
            print("push updates to firestore exception:" + str(e))
...
if __name__ == '__main__':
    ...
    firebase_queue = queue.Queue(maxsize=20)
    ...
    threadMain = threading.Thread(target=main,name="main")
    threadPushFirestore=threading.Thread(target=push_updates_to_firestore,name="pushfirestore")
    threadMain.start()
    threadPushFirestore.start()
    threadMain.join()
    threadPushFirestore.join()
